...
void insert_arr_tab(char* name, int upper_bound)
{
	int i;
...
SYM *lookup_arr_tab(char* name, int offset_arr)
{
	SYM *t ;
	t = arr_tab;
	while (t != NULL)
	{
		if (t->name==name && t->offset_array==offset_arr)
			return t;
		else 
			t = t->next;
	}
...
EXP *do_array(SYM *var, EXP *exp)
{
	...
	if(exp->ret->value<var->upper_bound&&exp->ret->value>=0)
	{
		...
		temp->prev=exp->tac;
		temp->a->value=lookup_arr_tab(var->name, exp->ret->value)->value;
