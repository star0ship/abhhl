...
void asm_array_copy(SYM *A, SYM *B)
{
	int reg1=get_first_reg(B); /* Load b into a register */
	insert_desc(reg1, A, MODIFIED); /* Indicate a is there */
...
{
	int reg1=get_first_reg(A);
	int reg2=get_second_reg(B,reg1);
	printf("	MUL R%u,4\n",reg2);
	printf("	ADD R%u,R%u\n",reg1,reg2);
	//printf("	LOD R%u,(R%u)\n",reg1,reg1);

	int reg3=get_third_reg(C,reg1,reg2);
	printf("	STO (R%u),R%u\n",reg1,reg3);
}
